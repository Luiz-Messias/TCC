...
export const useProdutoStore = defineStore('produto', () => {
  ...
  const criarProduto = async (dadosProduto) => {
    loading.value = true
    try {
      const resposta = await api.post('/api/produtos', dadosProduto)
  ...
  const atualizarProduto = async (id, dadosProduto) => {
    loading.value = true
    try {
      const resposta = await api.put(`/api/produtos/${id}`, dadosProduto)
  ...
  const carregarCategorias = async () => {
    try {
      ...
      // Para carregar todas as categorias, usar endpoint sem paginação ou pegar apenas os items
      categorias.value = resposta.data.data?.items || resposta.data.data || resposta.data
    } catch (erro) {
  ...
  const carregarMarcas = async () => {
    try {
      const resposta = await api.get('/api/marcas')
      // Para carregar todas as marcas, usar endpoint sem paginação ou pegar apenas os items
      marcas.value = resposta.data.data?.items || resposta.data.data || resposta.data
    } catch (erro) {
